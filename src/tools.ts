...
import type { EgoBrowserEngine, RunResult } from './engine.js'
...
function renderRun(result: RunResult): string {
  ...
  if (result.truncated) parts.push('(output was truncated by the capture cap)')
...
const RUN_OUTPUT = {
  ...
  render: (_args: Record<string, unknown>, value: RunResult) => text(renderRun(value)),
} as const

/** The script-execution tool — the faithful port of the skill's Bash heredoc. */
export function egoBrowserRunTool(engine: EgoBrowserEngine) {
  return defineTool({
...
export function egoBrowserHelpTool() {
  return defineTool({
    ...
    output: {
      ...
      render: (_args, value: { topic: string; text: string }) => text(value.text),
    },
    async execute(args) {
...
export function egoBrowserStatusTool(engine: EgoBrowserEngine) {
  return defineTool({
